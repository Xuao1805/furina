...
static void chassis_init(chassis_move_t *chassis_move_init);   //初始化各变量
static void chassis_set_mode(chassis_move_t *chassis_move_mode);   //底盘控制模式设置
void chassis_mode_change_control_transit(chassis_move_t *chassis_move_transit);    //底盘模式改变
static void chassis_feedback_update(chassis_move_t *chassis_move_update);    //底盘反馈数据更新
static void chassis_set_contorl(chassis_move_t *chassis_move_control);    //数据指针    
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop);   //电机电流值循环控制
...
//    chassis_move_init->chassis_pitch_motor = get_pitch_motor_point();
    ...
    for (i = 0; i < 4; i++)
    {
        chassis_move_init->motor_chassis[i].chassis_motor_measure = get_trigger_motor_measure_point(i);
